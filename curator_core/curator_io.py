...
def curation_master(fq, options):
    ...
    print("Processing " + fq_pref + " ... ")

    #---mapping---
    print("Running minimap2 for {}...".format(fq_pref))
    Path("{}.minimap2_running".format(fq)).touch()
    cmd = options.minimap2 + " -ax splice " + \
          check_mapping_db(options) + " " + fq
    code_msg, out_msg, err_msg = sys_run(cmd)
    print("Finished running minimap2 for {}.".format(fq_pref))
    del_cmd = "rm " + os.path.join("{}.minimap2_running".format(fq))
    os.system(del_cmd)
